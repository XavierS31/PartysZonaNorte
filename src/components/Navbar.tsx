...
const links = [
  ['Decoración', '/categories'],
  ['Globos', '/products?category=Globos'],
  ['Anchetas', '/products'],
  ['Galería', '/gallery'],
  ['Contacto', '/contact'],
];
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-butter/30 bg-white">
      <nav className="shell flex h-24 items-center justify-between gap-4" aria-label="Navegación principal">
        ...
        <div className="hidden items-center gap-3 sm:flex">
          <Link
            ...
            to="/products"
            className="rounded-full border-2 border-butter bg-white p-2 shadow-pink transition hover:scale-105"
          >
            <ShoppingBag size={19} />
          </Link>
        ...
        <button
          className="rounded-full border-2 border-butter p-2 shadow-pink lg:hidden"
      ...
      {open && (
        <div className="shell border-t border-butter/30 py-4 lg:hidden">
